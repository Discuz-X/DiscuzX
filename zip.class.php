...
class Zip {
	...
	*/
	...
	并返回文件的写入权限.
	*/
	function startfile($path = 'dpu.zip') {
	...
	*/
	...
	function createfile() {
		...
		$endstr = "\x50\x4b\x05\x06";
		$endstr .= "\x00\x00";
		$endstr .= "\x00\x00";
		$endstr .= pack('v', $this->file_count);
		$endstr .= pack('v', $this->file_count);
		$endstr .= pack('V', $this->dirstr_len);
		$endstr .= pack('V', $this->datastr_len);
		$endstr .= "\x00\x00";
...
class Unzip {
	...
	function ReadFileHeader($zip) {
		...
		,'vfilename_len'
		,'vextra_len');
		$data = unpack(implode('/',$headerSign), $binary_data);
	...
	function ReadCentralFileHeaders($zip) {
		...
		,'Vexternal'
		,'Voffset');
		$header = unpack(implode('/', $headerSign), $binary_data);

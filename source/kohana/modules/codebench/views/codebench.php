...
<form id="runner" method="post" action="<?php echo URL::site('codebench') ?>">
	<h1>
		<input name="class" type="text" value="<?php echo ($class !== '') ? $class : 'Bench_' ?>" size="25"
			   title="Name of the Codebench library to run"/>
			...
			<?php foreach($codebench['benchmarks'] as $method => $benchmark) { ?>
				<li>
					...
					<h2 title="<?php printf('%01.6f', $benchmark['time']) ?>s">
						<span class="grade-<?php echo $benchmark['grade']['time'] ?>"
							  style="width:<?php echo $benchmark['percent']['slowest']['time'] ?>%">
					...
					<div>
						<table>
							...
							<tr>
								...
								<th class="numeric" style="width:25%"
									title="Total method memory"><?php echo Text::bytes($benchmark['memory'], 'MB', '%01.6f%s') ?></th>
								<th class="numeric" style="width:25%"
									title="Total method time"><?php printf('%01.6f', $benchmark['time']) ?>s
								</th>
							</tr>
							</thead>
							<tbody>
							...
							<?php foreach($benchmark['subjects'] as $subject_key => $subject) { ?>
								<tr>
									<td>
										<strong class="help"
												title="(<?php echo gettype($codebench['subjects'][$subject_key]) ?>) <?php echo HTML::chars(var_export($codebench['subjects'][$subject_key], TRUE)) ?>">
									...
									<td class="numeric">
										<span
											title="+<?php echo (int)$subject['percent']['fastest']['memory'] ?>% memory">
...
<p id="footer">
	...
	<a href="http://github.com/kohana/codebench">Codebench</a>, a <a href="http://kohanaframework.org/">Kohana</a>
	module

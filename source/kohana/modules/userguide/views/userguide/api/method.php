<div class="method">
	...
	<h3 id="<?php echo $doc->method->name ?>">
		...
		<small>(defined
			in <?php echo html::anchor($route->uri(array('class' => $declares->name)), $declares->name, NULL, NULL, TRUE) ?>
			)
		</small>
	</h3>
	...
	<?php if($doc->return): ?>
		...
		<ul class="return">
			<?php foreach($doc->return as $set): list($type, $text) = $set; ?>
				<li>
					<code><?php echo HTML::chars($type) ?></code><?php if($text) echo ' - '.HTML::chars(ucfirst($text)) ?>
				</li>
			<?php endforeach ?>
		</ul>
	<?php endif ?>

<h1>
	...
	<?php foreach($doc->parents as $parent): ?>
		<br/>
		<small>
			extends <?php echo HTML::anchor($route->uri(array('class' => $parent->name)), $parent->name, NULL, NULL, TRUE) ?></small>
	<?php endforeach; ?>
</h1>
		...
		<ul>
			<?php if($properties = $doc->properties()): ?>
				<?php foreach($properties as $prop): ?>
					<li><a href="#property:<?php echo $prop->property->name ?>">$<?php echo $prop->property->name ?></a>
					</li>
				<?php endforeach ?>
			<?php else: ?>
				<li><em><?php echo 'None'; ?></em></li>
...
<?php if($properties = $doc->properties()): ?>
	...
	<div class="properties">
		<dl>
			<?php foreach($properties as $prop): ?>
				<dt><h4 id="property:<?php echo $prop->property->name ?>"><?php echo $prop->modifiers ?>
					<code><?php echo $prop->type ?></code> $<?php echo $prop->property->name ?></h4></dt>

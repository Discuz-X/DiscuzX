...
function category_get_list($cat, $wheresql, $page = 1, $perpage = 0) {
	...
	if($count) {
		$query = C::t('portal_article_title')->fetch_all_by_sql($wheresql, 'ORDER BY at.dateline ' . ($cat['asclist'] ? 'ASC' : 'DESC'), $start, $perpage, 0, 'at');
	...
	foreach ($cachearr as $key) {
		...
		if($data[$key] === false) {
			...
			if($key == 'portalhotarticle') {
				...
				$query = C::t('portal_article_title')->fetch_all_by_sql($wheresql, 'ORDER BY at.dateline ' . ($cat['asclist'] ? 'ASC' : 'DESC'), 0, 10, 0, 'at');
				...
				if(!empty($_G['cache']['portalcategory'][$cacheid])) {
					...
					$query = C::t('portal_article_title')->fetch_all_by_sql($where, 'ORDER BY at.dateline ' . ($cat['asclist'] ? 'ASC' : 'DESC'), 0, 10, 0, 'at');
				}
			}

	...
	foreach(C::t('common_nav')->fetch_all_mainnav() as $nav) {
		...
		if(!($nav['identifier'] == 5 && $nav['type'] == 0)) {
			foreach(C::t('common_nav')->fetch_all_subnav($nav['id']) as $subnav) {
				$item = "<a href=\"$subnav[url]\" hidefocus=\"true\" ".($subnav['title'] ? "title=\"$subnav[title]\" " : '').'target="'.vtarget($subnav['target']).'" '.parsehighlight($subnav['highlight']).">$subnav[name]</a>";
		...
		$data['navs'][$id]['nav'] = "id=\"$navid\" ".($onmouseover ? 'onmouseover="'.$onmouseover.'"' : '')."><a href=\"$nav[url]\" hidefocus=\"true\" ".($nav['title'] ? "title=\"$nav[title]\" " : '').'target="'.vtarget($nav['target']).'" '." $nav[style]>$nav[name]".($nav['identifier'] == 5 && $nav['type'] == 0 ? '<b class="icon_down"></b>' : '')."</a";
	}
	...
	foreach(C::t('common_nav')->fetch_all_by_navtype(2) as $nav) {
		...
		if($nav['name'] != '{hr}') {
			if(in_array($nav['name'], array('{userpanelarea1}', '{userpanelarea2}'))) {
				...
				$nav['code'] = '<li>'.$nav['subcode'].'<a href="'.$nav['url'].'"'.($nav['title'] ? ' title="'.$nav['title'].'"' : '').' target="'.vtarget($nav['target']).'"'.'>'.$nav['icon'].$nav['name'].'</a></li>';
			}
		} else {
...
function get_cachedata_mynavs() {
	...
	foreach(C::t('common_nav')->fetch_all_by_navtype(3) as $nav) {
		...
		$nav['code'] = '<a href="'.$nav['url'].'"'.($nav['title'] ? ' title="'.$nav['title'].'"' : '').' target="'.vtarget($nav['target']).'"'.$nav['icon'].'>'.$nav['name'].'</a>';
		$id = $nav['type'] == 0 ? $nav['identifier'] : 100 + $nav['id'];
...
function get_cachedata_topnav() {
	...
	foreach(C::t('common_nav')->fetch_all_by_navtype(4) as $nav) {
		...
		$nav['code'] = '<a href="'.$nav['url'].'"'.($nav['title'] ? ' title="'.$nav['title'].'"' : '').' target="'.vtarget($nav['target']).'"'.' '.parsehighlight($nav['highlight']).$nav['extra'].'>'.$nav['name'].'</a>';
		$id = $nav['type'] == 0 ? $nav['identifier'] : 100 + $nav['id'];
